...
//			String vmm = "Xen"; // VMM name
			...
			if (ValueHolder.vmConfigurations != null) {
				for (int i = 0; i < ValueHolder.vmConfigurations.size(); i++) {
					...
					int vmID = vmConf.vmId;
					Vm scopedVMObject = new Vm(vmConf.vmId, brokerId, vmConf.vmMips, vmConf.vmPesNumber,
...
//					Vm v= vmlist.get(ValueHolder.cloudletConfigurations.get(i).vmId);
					int vgetid = ValueHolder.cloudletConfigurations.get(i).vmId;
					scopedCloudlet.setVmId(vgetid);
...
//			cloudlet.setVmId();
	...
	private static Datacenter createDatacenter(String name) {
		...
		//Saurabh Lets have 16 PE units i.e processing elements each having *mips* MIPS
		for(int i=0; i<16; i++)
		...
		// of machines
		int hostId = 0;
		int ram =ConfigurationContainer.DataCenter.hostMemRam; // host memory (MB) Saurabh : 32 GB
		long storage = ConfigurationContainer.DataCenter.hostStorage; // host storage
		int bw = ConfigurationContainer.DataCenter.dcBw; // Saurabh 30 GB
		...
		for(int i=1; i<ConfigurationContainer.DataCenter.num_hosts; i++)
		{
			//create another machine in the Data center
			List<Pe> peListN = new ArrayList<Pe>();

			peListN.add(new Pe(0, new PeProvisionerSimple(mips)));
			...
			hostList.add(
	    			new Host(
	    				...
	    				storage,
	    				peListN,
	    				new VmSchedulerTimeShared(peListN)
	    			)
	...
	private static String printCloudletList(List<Cloudlet> list) {
		...
		int commaIndex = Json.lastIndexOf(",");
		if(commaIndex>-1)
			Json = Json.substring(0, commaIndex);
		Json +=" ] } ";
