...
public class RunSimulation {
	...
	static String run() {
		String log = "Starting Simulation ";
		Log.printLine("Starting CloudSimExample5...");

		try {
			// First step: Initialize the CloudSim package. It should be called
			// before creating any entities.
			int num_user = ConfigurationContainer.ValueHolder.num_users; // number of cloud users
			...
			if (ValueHolder.vmConfigurations != null) {
				for (int i = 0; i < ValueHolder.vmConfigurations.size(); i++) {
					...
					Vm scopedVMObject = new Vm(vmConf.position, brokerId1, vmConf.vmMips, vmConf.vmPesNumber,
							vmConf.vmRam, vmConf.vmBw, vmConf.vmSize, vmConf.vmm, new CloudletSchedulerTimeShared());
					vmlist1.add(scopedVMObject);
				}
			}
			...
			String jsonResponse = "{ \"array\": [";

			Log.print("=============> User " + brokerId1 + "    ");
			jsonResponse += printCloudletList(newList1);

			jsonResponse += ",";

			Log.print("=============> User " + brokerId2 + "    ");
			jsonResponse += (printCloudletList(newList2) + "] }");
			return jsonResponse;
